...
export default function DownloadPage() {
  ...
  const extractVideoId = (input) => {
    ...
    return String(input || '').trim().substring(0, 11);
  };
  ...
  const forceRefreshIfNeeded = useCallback(
    async () => {
      ...
      try {
        ...
        if (isResetOverdue) {
          loggerSafe(() =>
            toast('🔄 Refreshing plan status (reset passed)…', { duration: 1200, id: 'reset-refresh' })
          );
        }
      } catch {
        // silent
  ...
  const handleDownload = async () => {
    try {
      ...
      // Auth expiry handling (very common in production)
      if (res.status === 401) {
      ...
      if (downloadType === 'transcript') {
        ...
        if (AUTO_START_DOWNLOAD && data.direct_download_url && !autoTriggeredRef.current) {
          ...
          try {
            const href = String(data.direct_download_url || '');
            const finalHref = href.startsWith('http') ? href : joinUrl(API_BASE_URL, href);
    ...
    return (
      <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
        <div className="text-center mb-4">
          ...
          <div className="text-xl font-semibold text-gray-800 mb-2">{downloadType === 'audio' ? 'Audio' : 'Video'} Ready</div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto p-6">
        {/* Brand header */}
        <div className="mb-6">
          <AppBrand size={32} showText={true} label="OneTechly — YCD" logoSrc="/logo_onetechly.png" to="/app/dashboard" />
        </div>
        ...
        <div className="text-center mb-6">
          ...
          <div className="text-sm text-gray-600 mb-2">
            Logged in as <span className="font-semibold text-blue-600">{user?.username || 'User'}</span> ({user?.email})
          </div>
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-4 mb-4 mt-4 shadow-sm">
              ...
              <div className="text-sm">
                ...
                <span
                  className={`ml-1 px-2 py-1 rounded text-xs font-medium ${
                    tier === 'free' ? 'bg-yellow-100 text-yellow-800' : tier === 'pro' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'
                  }`}
                >
            ...
            {subscriptionStatus?.next_reset && (
              <div className="mt-1 text-xs text-gray-500">
                ...
                {isResetOverdue && <span className="ml-2 text-red-600 font-semibold">(reset passed — refreshing plan status)</span>}
              </div>
            )}
          </div>
        </div>
        ...
        {/* Input */}
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-700 mb-2">Enter YouTube Video ID or URL:</label>
          <input
            type="text"
        ...
        {/* Errors */}
        {getUsageLimitMessage() && <div className="bg-red-50 border border-red-200 text-red-700 p-4 rounded-lg mb-4 shadow-sm">⚠️ {getUsageLimitMessage()}</div>}
        {error && <div className="bg-red-50 border border-red-200 text-red-700 p-4 rounded-lg mb-4 shadow-sm">⚠️ {error}</div>}

        {/* Action buttons */}
        <div className="flex gap-4 mb-6">
          <button onClick={handleDownload} disabled={xUiPrimaryDisabled} aria-disabled={xUiPrimaryDisabled} title={xUiDisabledReason()} className={primaryBtnClass}>
            {isLoading
              ? '⏳ Processing…'
              : downloadType === 'transcript'
              ? '📄 Get Transcript'
        ...
        {(result || downloadUrl) && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            <h2 className="text-xl font-semibold mb-4 text-gray-900 flex items-center">
              ...
              {downloadType !== 'transcript' && downloadStarted && <span className="ml-2 text-green-600 text-sm font-normal">✅ Download Complete</span>}
            </h2>
