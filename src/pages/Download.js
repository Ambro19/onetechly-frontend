...
export default function DownloadPage() {
  ...
  const [downloadType, setDownloadType] = useState('transcript'); // transcript | audio | video
  const [transcriptType, setTranscriptType] = useState('clean');  // clean | unclean
  const [uncleanFormat, setUncleanFormat] = useState('srt');       // srt | vtt
  ...
  // ---- Short polling to ensure UI usage syncs after backend increments ----
  const pollUsageSync = useCallback(async (beforeUsageMap, keyToObserve) => {
    // Try up to 6 times over ~3 seconds to see usage increment reflected
    // This fixes “mismatch” cases (e.g., Onetechly).
    const maxTries = 6;
    let tries = 0;
    ...
    while (tries < maxTries && !pollStopRef.current) {
      tries += 1;
      try {
        await refreshSubscriptionStatus(); // triggers fetch & state update
      } catch {}
  ...
  const handleDownload = async () => {
    try {
      ...
      // Snapshot usage before call (so we can detect increments)
      const beforeUsage = {
      ...
      if (downloadType === 'transcript') {
        ...
        // Try to sync usage immediately after
        ...
        // Auto start the browser file download (desktop vs mobile)
        if (AUTO_START_DOWNLOAD && data.direct_download_url && !autoTriggeredRef.current) {
          ...
          try {
            ...
            // If auto-download fails, keep link visible for manual click
          }
        }

        // Try to sync usage increment for audio/video
      ...
      setIsLoading(false);
      // One more refresh after action completes
      try {
  ...
  const renderTranscript = () => {
    ...
    // unclean (timestamped) — detect VTT/SRT for nicer view
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto p-6">
        ...
        <div className="text-center mb-6">
          ...
          <button onClick={handleLogout} className="text-sm text-red-600 hover:text-red-800 underline transition-colors">
            Logout
          </button>
          {successMessage && <span className="sr-only" aria-live="polite">{successMessage}</span>}

          {/* Usage Summary (with reset date) */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <label className={`border-2 rounded-lg p-4 cursor-pointer transition-all ${downloadType === 'transcript' ? 'border-blue-500 bg-blue-50 shadow-sm' : 'border-gray-200 hover:border-gray-300'}`}>
          ...
          <label className={`border-2 rounded-lg p-4 cursor-pointer transition-all ${downloadType === 'audio' ? 'border-green-500 bg-green-50 shadow-sm' : 'border-gray-200 hover:border-gray-300'}`}>
          ...
          <label className={`border-2 rounded-lg p-4 cursor-pointer transition-all ${downloadType === 'video' ? 'border-purple-500 bg-purple-50 shadow-sm' : 'border-gray-200 hover:border-gray-300'}`}>
        ...
        {downloadType === 'transcript' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <label className={`border-2 rounded-lg p-4 cursor-pointer transition-all ${transcriptType === 'clean' ? 'border-blue-500 bg-blue-50 shadow-sm' : 'border-gray-200 hover:border-gray-300'}`}>
            ...
            <label className={`border-2 rounded-lg p-4 cursor-pointer transition-all ${transcriptType === 'unclean' ? 'border-blue-500 bg-blue-50 shadow-sm' : 'border-gray-200 hover:border-gray-300'}`}>
              ...
              {transcriptType === 'unclean' && (
                <div className={`pl-4 mt-2 space-y-2 border-l-2 border-blue-200 ${uncleanAtLimit ? 'opacity-50' : ''}`}>
                  {['srt', 'vtt'].map((fmt) => (
                    <label key={fmt} className={`flex items-center ${uncleanAtLimit ? 'cursor-not-allowed' : ''}`}>
                      <input
                        type="radio"
                        name="uncleanFormat"
                        value={fmt}
        ...
        {(result || downloadUrl) && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            <h2 className="text-xl font-semibold mb-4 text-gray-900 flex items-center">
              {downloadType === 'transcript' ? '📜 Transcript Result' : downloadType === 'audio' ? '🎵 Audio Result' : '🎬 Video Result'}
...
// // src/pages/Download.js — Production-polished with professional UI consistency
// // - Aligns "Video Quality" as segmented control (consistent with provided screenshots)
// // - Accessible radios (fieldset/legend/labels), mobile-friendly, equal widths
// // - FIXED: Consistent usage display with proper tier limits
// // - FIXED: Onetechly user display matching dashboard requirements
...
//   useEffect(() => {
//     if (refreshSubscriptionStatus && isAuthenticated) {
//       setIsRefreshingSubscription(true);
//       refreshSubscriptionStatus()
//         .catch(console.error)
//         .finally(() => setIsRefreshingSubscription(false));
//     }
//   }, [refreshSubscriptionStatus, isAuthenticated]);
...
//     }
//   }, [usageUpdated, refreshSubscriptionStatus]);
...
//       try {
//         await refreshSubscriptionStatus();
//         toast.success('Subscription status refreshed!', { duration: 2000 });
//       } catch (error) {
...
//   const limitInfo = useMemo(() => (subscriptionStatus?.limits || {}), [subscriptionStatus]);
//   const usageInfo = useMemo(() => (subscriptionStatus?.usage || {}), [subscriptionStatus]);

//   // FIXED: Enhanced usage formatting that handles all tier limits correctly
//   const safeFormatUsage = (actionType) => {
//     // First try the context's formatUsage function
//     if (typeof formatUsage === 'function') {
//       const result = formatUsage(actionType);
//       if (result && result !== '0 / 0') return result;
//     }
    
//     // Fallback to manual calculation with correct tier limits
//     const usage = subscriptionStatus?.usage?.[actionType] || 0;
//     const currentTier = tier || 'free';
    
//     // Define correct limits per tier (matching dashboard requirements)
//     const tierLimits = {
//       free: {
//         clean_transcripts: 5,
...
//     const limit = tierLimits[currentTier]?.[actionType] || tierLimits.free[actionType];
...
//   const uiHasReachedLimit = useCallback(
//     (action) => {
//       if (typeof hasReachedLimit === 'function') return !!hasReachedLimit(action);
//       const limit = limitInfo[action];
//       const used = usageInfo[action] || 0;
//       if (limit === 'unlimited' || limit === Infinity || limit === undefined || limit === null) return false;
//       return Number(used) >= Number(limit);
//     },
//     [hasReachedLimit, limitInfo, usageInfo]
//   );

//   const audioAtLimit = uiHasReachedLimit('audio_downloads');
//   const videoAtLimit = uiHasReachedLimit('video_downloads');
//   const uncleanAtLimit = uiHasReachedLimit('unclean_transcripts');

//   const canDownload = () => {
//     if (downloadType === 'transcript') {
//       const actionType = transcriptType === 'clean' ? 'clean_transcripts' : 'unclean_transcripts';
//       return canPerformAction && canPerformAction(actionType);
//     }
//     if (downloadType === 'audio') return canPerformAction && canPerformAction('audio_downloads');
//     if (downloadType === 'video') return canPerformAction && canPerformAction('video_downloads');
//     return false;
//   };
...
//     if (downloadType === 'transcript') {
//       const actionType = transcriptType === 'clean' ? 'clean_transcripts' : 'unclean_transcripts';
//       return hasReachedLimit && hasReachedLimit(actionType)
//         ? `Monthly limit reached for ${transcriptType} transcripts. Please upgrade your plan.`
//         : null;
//     }
//     if (downloadType === 'audio') {
//       return hasReachedLimit && hasReachedLimit('audio_downloads')
//         ? 'Monthly limit reached for audio downloads. Please upgrade your plan.'
//         : null;
//     }
//     if (downloadType === 'video') {
//       return hasReachedLimit && hasReachedLimit('video_downloads')
//         ? 'Monthly limit reached for video downloads. Please upgrade your plan.'
//         : null;
//     }
//     return null;
//   };
...
//         setDownloadCompleted(true);
//         toast.success('📄 Transcript ready');
//         if (data.usage_updated !== undefined) setUsageUpdated(true);
//       } else {
//         setDownloadUrl(data.direct_download_url);
//         const meta = {
//           title: data.title || 'Unknown Title',
...
//           autoTriggeredRef.current = true;
//           if (isMobile()) {
//             window.open(`${API_BASE_URL}${data.direct_download_url}`, '_blank');
//           } else {
//             const a = document.createElement('a');
//             a.href = `${API_BASE_URL}${data.direct_download_url}`;
//             a.download = meta.filename || '';
//             a.rel = 'noopener';
//             a.target = '_self';
//             document.body.appendChild(a);
...
//       setTimeout(() => refreshSubscriptionStatus && refreshSubscriptionStatus(), 900);
//     } catch (err) {
...
//   const xUiValidId = !!previewVideoId && previewVideoId.length === 11;
//   const xUiDisabledReason = () => {
//     if (downloadType === 'audio' && audioAtLimit) return 'Monthly limit reached for audio downloads. Please upgrade your plan.';
//     if (downloadType === 'video' && videoAtLimit) return 'Monthly limit reached for video downloads. Please upgrade your plan.';
//     if (downloadType === 'transcript' && transcriptType === 'unclean' && uncleanAtLimit) return 'Monthly limit reached for unclean transcripts. Please upgrade your plan.';
//     if (!xUiValidId) return 'Enter a valid YouTube URL or ID';
//     return '';
//   };
//   const xUiPrimaryDisabled =
//     isLoading ||
//     !xUiValidId ||
//     (downloadType === 'audio' && audioAtLimit) ||
//     (downloadType === 'video' && videoAtLimit) ||
//     (downloadType === 'transcript' && transcriptType === 'unclean' && uncleanAtLimit) ||
//     !canDownload();
...
//           {/* Usage Summary - FIXED: Now shows consistent usage with dashboard */}
//           <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-4 mb-4 mt-4 shadow-sm">
//             <div className="flex items-center justify-between mb-2">
//               <div className="text-sm">
//                 <span className="font-semibold">Current Plan:</span>{' '}
...
//                   {tier?.charAt(0).toUpperCase() + tier?.slice(1) || 'Free'}
//                 </span>
//                 {usageUpdated && <span className="ml-2 text-green-600 text-xs font-medium">✅ Updated</span>}
//               </div>
//               <button
...
///////// VERY GOOD DOWNLOAD.JS FILE. KEEP IT!!! //////////// 
...
//   const safeFormatUsage = (actionType) => {
//     if (typeof formatUsage === 'function') return formatUsage(actionType);
//     if (subscriptionStatus?.usage && subscriptionStatus?.limits) {
//       const u = subscriptionStatus.usage[actionType] || 0;
//       const l = subscriptionStatus.limits[actionType];
//       return l === 'unlimited' || l === Infinity ? `${u} / ∞` : `${u} / ${l}`;
//     }
//     return '0 / 0';
//   };
