    ...
    <div className="min-h-screen bg-gray-50">
      <div className="mx-auto w-full max-w-4xl px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="flex flex-col items-center text-center mb-6">
          ...
          <h1 className="mt-3 text-3xl font-bold text-gray-900">💳 Subscription</h1>
          ...
          <button
            ...
            className="mt-3 bg-gray-700 text-white px-4 py-2 rounded-lg hover:bg-gray-800 transition-colors focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2"
          >
            ← Back to Dashboard
          </button>
        </div>
...
// // ✅ Default export: SubscriptionPage
// export default function SubscriptionPage() {
//   const navigate = useNavigate();
//   const [searchParams, setSearchParams] = useSearchParams();
//   const { token, user, isAuthenticated } = useAuth();
...
//   // safety guards for async state updates
//   const mountedRef = useRef(true);
//   const pollTimeoutRef = useRef(null);
...
//       checkPaymentSuccess();
//     }
//   }, [isAuthenticated, refreshSubscriptionStatus, checkPaymentSuccess]);

//   const planTier = (tier || 'free').toLowerCase();
//   const isActive = planTier !== 'free';

//   // ---------- usage formatters (clamp to limits for UI coherence) ----------
...
//   const fmtUsage = (k) => {
//     const u = subscriptionStatus?.usage?.[k] ?? 0;
//     const l = subscriptionStatus?.limits?.[k];
//     if (l === 'unlimited' || l === Infinity) return `${u} / ∞`;
//     return `${clampCount(u, l)} / ${l ?? 0}`;
//   };
...
//     // For existing subscribers route to the portal
//     if (planTier !== 'free') {
//       toast.loading('Opening billing portal for subscription changes...', { duration: 2000 });
//       await openBillingPortal();
//       return;
...
//   // ---------- usage formatters (clamp to limits for UI coherence) ----------
//   const clampCount = (usage, limit) => {
//     if (limit === 'unlimited' || limit === Infinity || limit == null) return usage;
//     return Math.min(Number(usage || 0), Number(limit || 0));
//   };
